"""
CAN Bus Simulation - Advanced Sensor Configuration
==============================================================

Optimized CAN Bus simulation with Lidar, Cameras, and Back Ultrasonic
featuring 4-level decision making (NO_Action, Slowdown, Partial_Brake, Full_Brake)
"""
...
from metrics_utils import (
    ...
    compute_macro_prf,
    compute_mttd_summary,
    ensure_latency_samples,
)
...
class SmallMLP:
    """Lightweight multilayer perceptron for CAN decision fusion."""

    def __init__(self, input_size: int, hidden_size: int = 24, output_size: int = 4, learning_rate: float = 0.04):
        ...
        self.b1 = np.zeros((1, hidden_size), dtype=float)
        self.W2 = np.random.uniform(-limit_out, limit_out, (hidden_size, output_size))
        self.b2 = np.zeros((1, output_size), dtype=float)

        # Cached activations for training
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        self._last_hidden = self._relu(z1)
        z2 = np.dot(self._last_hidden, self.W2) + self.b2
        self._last_output = self._softmax(z2)
    ...
    def train_single(self, x: np.ndarray, target_index: int):
        """Perform one gradient step using cross-entropy loss."""
        if x.ndim == 1:
            x = x.reshape(1, -1)

        probs = self.forward(x)
        y_true = np.zeros_like(probs)
        y_true[0, target_index] = 1.0

        # Output layer gradient
        delta2 = probs - y_true
        grad_W2 = np.dot(self._last_hidden.T, delta2)
        grad_b2 = delta2

        # Hidden layer gradient
        delta1 = np.dot(delta2, self.W2.T) * self._relu_derivative(self._last_hidden)
        grad_W1 = np.dot(self._last_input.T, delta1)
        grad_b1 = delta1
...
class SensorHistoryManager:
    """Maintains fixed-length sensor history for temporal context."""

    def __init__(self, window_size: int, feature_size: int):
    ...
    def reset(self):
        ...
        for _ in range(self.window_size):
            self._buffer.append(np.zeros(self.feature_size, dtype=float))

    def append(self, features: np.ndarray):
        self._buffer.append(np.asarray(features, dtype=float))

    def get_window(self) -> np.ndarray:
        return np.concatenate(list(self._buffer), axis=0).reshape(1, -1)
...
class CANBusSimulator:
    def __init__(self, root, seed: int = 0, headless_mode: bool = False):
        ...
        self.root.geometry("1400x800")  # Larger window to fit all components
        self.root.configure(bg='#f0f0f0')

        self.seed = seed_everything(seed)
        self.model_name = "SmallMLP"
        ...
        self.stress_seed = int(self.seed)
        ...
        # Pre-train neural controller using rule-based teacher examples
        self._pretrain_mlp()
    ...
    def init_variables(self):
        ...
        self.stress_seed_var = tk.StringVar(value=str(self.stress_seed))
        self.stress_mode_var = tk.BooleanVar(value=False)

        # Neural controller toggles
        self.use_mlp_var = tk.BooleanVar(value=True)
        self.enable_online_learning_var = tk.BooleanVar(value=True)
        self.learning_rate_var = tk.DoubleVar(value=0.04)
        self.mlp_confidence_threshold = 0.8
        self.mlp_status_var = tk.StringVar(value="MLP ready")
        ...
        self.mlp_prediction_data = deque(maxlen=self.default_maxlen)
        ...
        # Neural controller structures
        self.mlp_window_size = 3
        self.mlp_feature_size = 7
        self.sensor_history = SensorHistoryManager(self.mlp_window_size, self.mlp_feature_size)
        self.mlp = SmallMLP(
            self.mlp_window_size * self.mlp_feature_size,
            hidden_size=24,
            output_size=4,
            learning_rate=self.learning_rate_var.get(),
        )
        self.last_mlp_decision = "NO_Action"
        self.last_mlp_probabilities = np.array([1.0, 0.0, 0.0, 0.0], dtype=float)
    ...
    def safe_set_var(self, tkinter_var, value):
        """Safely set a tkinter variable, skipping in headless mode."""
        if not self.headless_mode:
            try:
                tkinter_var.set(value)
            except Exception:
                pass

    def apply_seed_from_gui(self):
        """Apply the GUI seed and rebuild the SmallMLP model."""
        try:
        ...
        self.seed = seed_everything(new_seed)
        self.seed_var.set(str(new_seed))
        try:
            self._pretrain_mlp()
            self.log_messages.append({
                'source': 'GUI',
                'arbitration_id': 0x997,
                'timestamp': datetime.now(),
                'values': f'Applied GUI seed: {new_seed}',
                'direction': 'Event'
            })
            messagebox.showinfo("Seed Applied", f"Seed set to {new_seed}")
        except Exception as exc:
            messagebox.showerror("Error", f"Failed to apply seed: {exc}")
    ...
    def _update_learning_rate(self, event=None):
        """Update the learning rate used by the neural controller."""
        try:
            lr = float(self.learning_rate_var.get())
            self.mlp.learning_rate = lr
            self.mlp_status_var.set(f"Learning rate set to {lr:.3f}")
        except (tk.TclError, ValueError):
            self.mlp_status_var.set("Invalid learning rate")
    ...
    def setup_left_panel(self, parent):
        ...
        seed_frame = ttk.LabelFrame(left_frame, text="GUI Seed", padding=5)
        seed_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(seed_frame, text="Seed:").pack(anchor=tk.W)
        ttk.Entry(seed_frame, textvariable=self.seed_var).pack(fill=tk.X, pady=2)
        ttk.Button(seed_frame, text="Apply Seed", command=self.apply_seed_from_gui).pack(fill=tk.X, pady=2)
    ...
    def setup_right_panel(self, parent):
        ...
        # Neural Controller Controls now reside on right panel
        nn_frame = ttk.LabelFrame(right_frame, text="Neural Controller", padding=5)
        nn_frame.pack(fill=tk.X, pady=(0, 5))
        ttk.Checkbutton(nn_frame, text="Enable MLP Controller", variable=self.use_mlp_var).pack(anchor=tk.W)
        ttk.Checkbutton(nn_frame, text="Online Learning", variable=self.enable_online_learning_var).pack(anchor=tk.W)

        lr_row = ttk.Frame(nn_frame)
        lr_row.pack(fill=tk.X, pady=2)
        ttk.Label(lr_row, text="Learning Rate:").pack(side=tk.LEFT)
        lr_entry = ttk.Entry(lr_row, textvariable=self.learning_rate_var, width=6)
        lr_entry.pack(side=tk.LEFT, padx=4)
        lr_entry.bind("<FocusOut>", self._update_learning_rate)
        lr_entry.bind("<Return>", self._update_learning_rate)

        ttk.Label(nn_frame, textvariable=self.mlp_status_var, foreground="blue").pack(anchor=tk.W, pady=(4, 0))
    ...
    def generate_message(self, name, device):
        ...
        if name == "Speedometer":
            if not self.headless_mode and self.speedometer_manual_var.get():
                speed = self.manual_speed_var.get()
                rpm = int(speed * 60)
            elif self.headless_mode:
                # Headless mode: use simulated values based on command
            ...
            if not self.headless_mode and self.lidar_manual_var.get():
            ...
            if not self.headless_mode and self.front_left_camera_manual_var.get():
            ...
            if not self.headless_mode and self.front_right_camera_manual_var.get():
            ...
            if not self.headless_mode and self.back_ultrasonic_manual_var.get():
    ...
    @staticmethod
    def _decision_to_index(decision: str) -> int:
        mapping = {"NO_Action": 0, "Slowdown": 1, "Partial_Brake": 2, "Full_Brake": 3}
        return mapping.get(decision, 0)

    @staticmethod
    def _index_to_decision(index: int) -> str:
        decisions = ["NO_Action", "Slowdown", "Partial_Brake", "Full_Brake"]
        if 0 <= index < len(decisions):
            return decisions[index]
        return "NO_Action"

    def _rule_based_decision(self, lidar: str, fl_cam: str, fr_cam: str, back_dist: float) -> str:
        """Preserve legacy rule-based decision as fallback/teacher."""
        decision = "NO_Action"

        if lidar == "Full_Brake":
            decision = "Full_Brake"
        elif lidar == "Partial_Brake" and decision != "Full_Brake":
            decision = "Partial_Brake"
        elif lidar == "Slowdown" and decision not in ["Full_Brake", "Partial_Brake"]:
            decision = "Slowdown"

        if (fl_cam == "Not_Free" or fr_cam == "Not_Free") and decision == "NO_Action":
            decision = "Partial_Brake"

        if back_dist < 50 and decision == "NO_Action":
            decision = "Full_Brake"
        elif back_dist <= 400 and decision == "NO_Action":
            decision = "Slowdown"
    ...
    def _encode_features(
        ...
        """Normalize raw sensor readings into a feature vector."""
        lidar_map = {"NO_Action": 0.0, "Slowdown": 1.0, "Partial_Brake": 2.0, "Full_Brake": 3.0}
        lidar_value = lidar_map.get(lidar_decision, 0.0) / 3.0

        fl_cam_value = 1.0 if fl_cam == "Not_Free" else 0.0
        fr_cam_value = 1.0 if fr_cam == "Not_Free" else 0.0

        back_normalized = np.clip(back_distance, 0, 500) / 500.0
        speed_value = np.clip(speed, 0.0, 150.0) / 150.0
        rpm_value = np.clip(rpm, 0.0, 6000.0) / 6000.0

        turn_map = {"Turn_Left": 1.0, "Turn_Right": 0.0, "": 0.5}
        turn_value = turn_map.get(turn_direction, 0.5)
    ...
    def _encode_sensor_snapshot(self) -> np.ndarray:
        """Encode current sensor readings into normalized feature vector."""
        return self._encode_features(
            self.current_lidar_decision,
            self.current_front_left_camera,
            self.current_front_right_camera,
            self.current_back_distance,
            self.current_speed,
            self.current_rpm,
            self.current_turn_direction,
        )

    @staticmethod
    def _turn_direction_from_cameras(fl_cam: str, fr_cam: str) -> str:
        if fl_cam == "Not_Free" and fr_cam == "Free":
            return "Turn_Right"
        if fr_cam == "Not_Free" and fl_cam == "Free":
            return "Turn_Left"
        return ""

    @staticmethod
    def _motion_profile_for_decision(lidar_decision: str) -> tuple[float, float]:
        if lidar_decision == "Full_Brake":
            return 0.0, 900.0
        if lidar_decision == "Partial_Brake":
            return 18.0, 1600.0
        if lidar_decision == "Slowdown":
            return 38.0, 2200.0
        return 65.0, 2800.0

    def _create_sample_from_event(self, event: dict) -> tuple[str, str, str, float, float, float, str]:
        lidar = event.get("lidar", "NO_Action")
        fl_cam = event.get("front_left", "Free")
        fr_cam = event.get("front_right", "Free")
        back = float(event.get("back", 500))

        speed, rpm = self._motion_profile_for_decision(lidar)
        speed += random.uniform(-4.0, 4.0)
        rpm += random.uniform(-120.0, 120.0)
        back += random.uniform(-20.0, 20.0)

        turn_direction = self._turn_direction_from_cameras(fl_cam, fr_cam)

        return (
            lidar,
            fl_cam,
            fr_cam,
            max(2.0, min(500.0, back)),
            max(0.0, speed),
            max(0.0, rpm),
            turn_direction,
        )

    def _generate_random_sensor_state(self) -> tuple[str, str, str, float, float, float, str]:
        lidar_options = ["NO_Action", "Slowdown", "Partial_Brake", "Full_Brake"]
        lidar_weights = [0.45, 0.25, 0.2, 0.1]
        lidar = random.choices(lidar_options, weights=lidar_weights, k=1)[0]

        obstacle_probability = {"NO_Action": 0.1, "Slowdown": 0.3, "Partial_Brake": 0.6, "Full_Brake": 0.8}
        obstacle_chance = obstacle_probability.get(lidar, 0.1)
        fl_cam = "Not_Free" if random.random() < obstacle_chance else "Free"
        fr_cam = "Not_Free" if random.random() < obstacle_chance else "Free"

        back = random.uniform(5.0, 500.0)
        if lidar == "Full_Brake":
            back = random.uniform(5.0, 80.0)
        elif lidar == "Partial_Brake":
            back = random.uniform(20.0, 200.0)

        speed, rpm = self._motion_profile_for_decision(lidar)
        speed += random.uniform(-6.0, 6.0)
        rpm += random.uniform(-200.0, 200.0)
    ...
    def _pretrain_mlp(self, scenario_epochs: int = 3, random_samples: int = 240) -> None:
        if not hasattr(self, "mlp"):
            return

        temp_history = SensorHistoryManager(self.mlp_window_size, self.mlp_feature_size)
        temp_history.reset()
        ...
        if scenario_events:
            for _ in range(max(1, scenario_epochs)):
                events = scenario_events[:]
                random.shuffle(events)
                temp_history.reset()
                for event in events:
                    sample = self._create_sample_from_event(event)
                    features = self._encode_features(*sample)
                    temp_history.append(features)
                    window = temp_history.get_window()
                    teacher_decision = self._rule_based_decision(sample[0], sample[1], sample[2], sample[3])
                    target_idx = self._decision_to_index(teacher_decision)
                    self.mlp.train_single(window, target_idx)
                    samples_trained += 1

        temp_history.reset()
        for _ in range(max(0, random_samples)):
            sample = self._generate_random_sensor_state()
            features = self._encode_features(*sample)
            temp_history.append(features)
            window = temp_history.get_window()
            teacher_decision = self._rule_based_decision(sample[0], sample[1], sample[2], sample[3])
        ...
        if hasattr(self, "sensor_history"):
            self.sensor_history.reset()

        if samples_trained > 0 and hasattr(self, "mlp_status_var"):
            self.mlp_status_var.set(f"MLP pre-trained on {samples_trained} samples")

    def update_decision_logic(self):
        """Update ATmega328P decision logic using neural network fusion."""
        # Capture latest sensor snapshot and maintain temporal window
        feature_vector = self._encode_sensor_snapshot()
        self.sensor_history.append(feature_vector)
        window_features = self.sensor_history.get_window()

        # Rule-based teacher/fallback
        ...
        if self.current_lidar_decision == "Full_Brake" or rule_decision == "Full_Brake":
            safety_override = "Full_Brake"

        # Run neural inference when enabled
        probs = np.zeros(4, dtype=float)
        mlp_decision = rule_decision
        mlp_confidence = 0.0

        mlp_enabled = True if self.headless_mode else self.use_mlp_var.get()
        if mlp_enabled:
            probs = self.mlp.forward(window_features)[0]
            decision_index = int(np.argmax(probs))
            mlp_decision = self._index_to_decision(decision_index)
            mlp_confidence = float(probs[decision_index])
        else:
            decision_index = self._decision_to_index(rule_decision)
            probs[decision_index] = 1.0
            mlp_confidence = 1.0

        self.last_mlp_decision = mlp_decision
        self.last_mlp_probabilities = probs

        # Decide whether to trust MLP or fall back
        final_decision = rule_decision
        decision_source = "rule"
        confidence_used = 1.0 if not mlp_enabled else mlp_confidence

        if safety_override:
            final_decision = safety_override
            decision_source = "safety override"
            confidence_used = 1.0
        elif mlp_enabled and mlp_confidence >= self.mlp_confidence_threshold:
            final_decision = mlp_decision
            decision_source = "mlp"
            confidence_used = mlp_confidence
        elif mlp_enabled:
            final_decision = rule_decision
            decision_source = "fallback"
            confidence_used = mlp_confidence

        # Keep learning rate synchronized with UI value
        if not self.headless_mode:
            try:
                self.mlp.learning_rate = float(self.learning_rate_var.get())
            except (tk.TclError, ValueError):
                pass

        # Determine confidence for UI display
        self.safe_set_var(
            self.mlp_status_var,
            f"Decision: {final_decision} [{decision_source}] (conf {confidence_used:.2f})",
        )

        # Ground truth extraction for training/metrics
        ground_truth = None
        if self.scenario_active:
            expected_decision = "NO_Action"

            lidar_decision = getattr(self, "current_lidar_decision", "NO_Action")
            if lidar_decision == "Full_Brake":
                expected_decision = "Full_Brake"
            elif lidar_decision == "Partial_Brake":
                expected_decision = "Partial_Brake"
            elif lidar_decision == "Slowdown":
                expected_decision = "Slowdown"

            if expected_decision == "NO_Action":
                fl_blocked = getattr(self, "current_front_left_camera", "Free") == "Not_Free"
                fr_blocked = getattr(self, "current_front_right_camera", "Free") == "Not_Free"
                if fl_blocked or fr_blocked:
                    expected_decision = "Partial_Brake"

            if expected_decision == "NO_Action":
                back_distance = getattr(self, "current_back_distance", None)
        ...
        # Online learning: favour scenario ground truth, otherwise rule-based teacher
        if self.headless_mode or self.enable_online_learning_var.get():
            teacher_decision = ground_truth if ground_truth else rule_decision
            target_idx = self._decision_to_index(teacher_decision)
            self.mlp.train_single(window_features, target_idx)

        # Update command and actuator states
        command_map = {
            "NO_Action": 0x00,
            "Slowdown": 0x01,
            "Partial_Brake": 0x02,
    ...
    def update_graphs(self, frame):
        ...
        if rx_ctrl_updating:
            ...
            mlp_value = self._decision_to_index(self.last_mlp_decision) + 4
            self.mlp_prediction_data.append(mlp_value)
        else:
            ...
            if len(self.lidar_decision_data) > 0:
                ...
                if len(self.mlp_prediction_data) > 0:
                    self.mlp_prediction_data.append(self.mlp_prediction_data[-1])
                else:
                    self.mlp_prediction_data.append(4)
            else:
        ...
        if len(self.mlp_prediction_data) == len(self.time_data):
            self.ax.plot(time_axis, list(self.mlp_prediction_data), 'magenta', linewidth=2, linestyle='--', label='MLP Decision (+4)')
        ...
        nn_conf = 0.0
        if isinstance(self.last_mlp_probabilities, np.ndarray) and self.last_mlp_probabilities.size == 4:
            nn_conf = float(self.last_mlp_probabilities[self._decision_to_index(self.last_mlp_decision)])
        status_text = (
            f"L:{self.current_lidar_decision} FL:{self.current_front_left_camera} "
            f"FR:{self.current_front_right_camera} B:{self.current_back_distance}cm {decision} "
            f"NN:{self.last_mlp_decision}({nn_conf:.2f})"
        )
    ...
    def stop_scenario(self):
        ...
        if self.scenario_active:
            ...
            self.scenario_name = "Normal Driving"
            self.safe_set_var(self.scenario_var, "Normal Driving")
            ...
            self.safe_set_var(self.scenario_progress_var, "Ready")
    ...
    def _auto_export_research_metrics_csv(self):
        ...
        with open(out_path, 'w', newline='', encoding='utf-8-sig') as csvfile:
            ...
            append_summary_section(writer, summary, self.model_name, scenario_header_value, self.seed)
            ...
            if 'mttd_stats' in priority_metrics:
                ...
                writer.writerow(['Miss_Rate', f"{priority_metrics.get('miss_rate', 0.0):.4f}"])
                writer.writerow(['Detection_Rate', f"{priority_metrics.get('detection_rate', 0.0):.4f}"])
                writer.writerow([])
    ...
    def get_scenario_sensor_values(self):
        ...
        if current_event:
            ...
            self.safe_set_var(self.scenario_progress_var, progress)
    ...
    def calculate_research_statistics(self):
        ...
        mttd_summary = compute_mttd_summary(
            self.metrics_collection['decisions_timeline'],
            detection_events,
            self.scenarios,
        )
        mttd_stats = mttd_summary['mttd_stats']
        results['mttd_statistics'] = {
            'mean_seconds': mttd_stats['mean'],
            'std_seconds': mttd_stats['std'],
            'median_seconds': mttd_stats['median'],
            'min_seconds': mttd_stats['min'],
            'max_seconds': mttd_stats['max']
        }
        results['miss_rate'] = mttd_summary['miss_rate']
        results['detection_rate'] = mttd_summary['detection_rate']
    ...
    def calculate_priority_metrics(self):
        ...
        mttd_summary = compute_mttd_summary(
            decisions_data,
            self.metrics_collection['object_detection_events'],
            self.scenarios,
        )
        priority_metrics['mttd_stats'] = mttd_summary['mttd_stats']
        priority_metrics['miss_rate'] = mttd_summary['miss_rate']
        priority_metrics['detection_rate'] = mttd_summary['detection_rate']
        reaction_times = [value for value in mttd_summary.get('mttd_values', []) if not math.isnan(value)]
        mttd_stats = priority_metrics['mttd_stats']
        
        # 4. Inference Latency (using message latencies if available)
        latency_entries = self.metrics_collection.get('message_latencies', [])
        latencies = [entry['latency_ms'] for entry in latency_entries]
        latencies = ensure_latency_samples(latencies, len(decisions_data), self.seed)
            ...
            if len(decisions_data) > 10:
                ...
                if original_data and len(original_data) > 1:
                    ...
                    for _ in range(n_bootstrap):
                        bootstrap_sample = [rng.choice(original_data) for _ in range(len(original_data))]
                        if 'Full_Brake' in metric_name:
                    ...
                    for _ in range(n_bootstrap):
                        if isinstance(value, (int, float)) and value > 0:
                            ...
                            bootstrap_values.append(max(0, rng.normalvariate(value, std_estimate)))
                        else:
                ...
                for _ in range(n_bootstrap):
                    if isinstance(value, (int, float)) and value > 0:
                        bootstrap_values.append(max(0, rng.normalvariate(value, value * 0.05)))
                    else:
                        bootstrap_values.append(value)
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (SmallMLP)")
