...
class TemporalSequenceModel:
    """Lightweight 1D temporal model using fixed convolutions and logistic output."""

    def __init__(self, sequence_length=12, feature_count=5, kernel_sizes=(3, 5), num_classes=4):
        self.sequence_length = sequence_length
        self.feature_count = feature_count
        self.kernel_sizes = kernel_sizes
        self.num_classes = num_classes
        self.kernels = self._init_kernels()
        self.feature_dim = feature_count * len(self.kernels) + feature_count + 1  # +1 for bias
        self.weights = np.zeros((self.feature_dim, num_classes), dtype=np.float32)
    ...
    def _init_kernels(self):
        ...
        for size in self.kernel_sizes:
            if size <= 0:
                continue
            base = np.linspace(0.25, 1.0, size, dtype=np.float32)
            base /= np.sum(base)
            kernels.append(base[::-1])  # emphasize recent history
        return kernels

    def extract_features(self, sequence):
        seq = np.asarray(sequence, dtype=np.float32)
        if seq.ndim != 2 or seq.shape[1] != self.feature_count:
            raise ValueError("Sequence must be shape (timesteps, feature_count)")
        ...
        for kernel in self.kernels:
            k = len(kernel)
            window = seq[-k:]
            conv = (window * kernel[:, None]).sum(axis=0)
            features.append(conv)

        features.append(seq[-1])  # most recent raw readings
        feature_vec = np.concatenate(features, axis=0)
        feature_vec = np.append(feature_vec, 1.0)  # bias
    ...
    def train_on_sequences(self, sequences, labels, epochs=20, lr=0.05, l2_reg=1e-4):
        if len(sequences) == 0:
            return

        features = np.array([self.extract_features(seq) for seq in sequences], dtype=np.float32)
        labels = np.asarray(labels, dtype=np.int32)
        num_samples = features.shape[0]

        one_hot = np.zeros((num_samples, self.num_classes), dtype=np.float32)
        ...
        for _ in range(max(1, epochs)):
            ...
            error = probs - one_hot
            gradient = (features.T @ error) / num_samples
            gradient += l2_reg * self.weights
            self.weights -= lr * gradient
        self.trained_steps += epochs


class CANBusSimulator:
    DECISION_ORDER = ["NO_Action", "Slowdown", "Partial_Brake", "Full_Brake"]
    DECISION_MAP = {name: idx for idx, name in enumerate(DECISION_ORDER)}
    ...
    def __init__(self, root, seed: int = 0, headless_mode: bool = False):
        ...
        # Setup temporal sequence model
        self.setup_sequence_model()
    ...
    def init_variables(self):
        ...
        self.sequence_model_enabled = tk.BooleanVar(value=True)
        self.model_status_var = tk.StringVar(value="Model: Initializing...")
        self.model_confidence_var = tk.StringVar(value="Confidence: N/A")
        self.model_ready = False
        default_features = self._encode_sensor_snapshot("NO_Action", "Free", "Free", 250, 60.0)
        self.sequence_history = deque(
            [default_features.copy() for _ in range(self.sequence_length)],
            maxlen=self.sequence_length
        )
        self.sequence_model = None
        self.sequence_training_buffer = deque(maxlen=2000)
    ...
    def apply_seed_from_gui(self):
        """Apply the GUI seed and rebuild the Sequence model."""
        try:
    ...
    def setup_right_panel(self, parent):
        ...
        # Sequence Model Controls relocated to right panel
        model_frame = ttk.LabelFrame(right_frame, text="Sequence Model", padding=5)
        model_frame.pack(fill=tk.X, pady=5)
        ttk.Checkbutton(model_frame, text="Enable Temporal Model", variable=self.sequence_model_enabled).pack(anchor=tk.W)
        ttk.Label(model_frame, textvariable=self.model_status_var, foreground="blue").pack(anchor=tk.W, pady=(4, 0))
        ttk.Label(model_frame, textvariable=self.model_confidence_var, foreground="darkgreen").pack(anchor=tk.W)
        ttk.Button(model_frame, text="Fine-tune from Scenario", command=self.train_model_from_scenario).pack(fill=tk.X, pady=(6, 2))
        ttk.Button(model_frame, text="Reset Model", command=self.reset_sequence_model).pack(fill=tk.X)
    ...
    def setup_sequence_model(self):
        """Initialize or refresh the temporal sequence model."""
        self._initialize_sequence_model(pretrain=True)

    def _initialize_sequence_model(self, pretrain=True):
        self.sequence_training_buffer.clear()
        try:
            self.sequence_model = TemporalSequenceModel(
                sequence_length=self.sequence_length,
                feature_count=5,
                kernel_sizes=(3, 5, 7)
            )
            if pretrain:
                sequences, labels = self._generate_synthetic_training_data(num_samples=2400)
                if sequences:
                    self.sequence_model.train_on_sequences(
                        sequences,
                        labels,
                        epochs=45,
                        lr=0.08,
                    ...
                    self.model_status_var.set(f"Model: Pretrained ({len(sequences)} synthetic seq)")
                else:
                    self.model_status_var.set("Model: Ready (no synthetic data)")
            else:
                self.model_status_var.set("Model: Reset (weights zeroed)")
            self.model_confidence_var.set("Confidence: N/A")
            self.model_ready = True
            self.sequence_model_enabled.set(True)
        except Exception as exc:
            self.model_status_var.set(f"Model init error: {str(exc)[:60]}")
            self.sequence_model_enabled.set(False)
            self.model_ready = False
            self.model_confidence_var.set("Confidence: N/A")

    def reset_sequence_model(self):
        """Reset model weights and re-run synthetic pretraining."""
        self._initialize_sequence_model(pretrain=True)
        messagebox.showinfo("Sequence Model", "Temporal model reset using synthetic data.")

    def train_model_from_scenario(self):
        """Fine-tune sequence model with collected scenario data."""
        if not self.sequence_model or not self.model_ready:
            messagebox.showwarning("Sequence Model", "Model is not initialized.")
            return

        if len(self.sequence_training_buffer) < 5:
            messagebox.showinfo("Sequence Model", "Insufficient scenario samples to train.")
            return

        sequences = [entry['sequence'] for entry in self.sequence_training_buffer]
        labels = [entry['label'] for entry in self.sequence_training_buffer]
        ...
        try:
            ...
            self.model_status_var.set(f"Model: Fine-tuned ({sample_count} scenario seq)")
            self.model_confidence_var.set("Confidence: N/A")
            messagebox.showinfo("Sequence Model", f"Fine-tuned on {sample_count} scenario samples.")
            self.sequence_training_buffer.clear()
        except Exception as exc:
            messagebox.showerror("Sequence Model", f"Training failed: {exc}")

    def _generate_synthetic_training_data(self, num_samples=2000):
        """Generate synthetic temporal sequences using rule-based labels."""
        sequences = []
        labels = []
        for _ in range(num_samples):
            lidar = random.choice(self.DECISION_ORDER)
            fl_cam = random.choice(["Free", "Not_Free"])
            fr_cam = random.choice(["Free", "Not_Free"])
            back_distance = random.uniform(40, 450)
            ...
            for _ in range(self.sequence_length):
                if random.random() < 0.35:
                    ...
                    if back_distance < 50:
                        lidar = random.choice(["Partial_Brake", "Full_Brake"])
                    elif back_distance < 400 and lidar == "NO_Action":
                        lidar = random.choice(["Slowdown", "Partial_Brake"])
                ...
                if random.random() < 0.3:
                    back_distance = float(np.clip(back_distance + random.uniform(-60, 60), 20, 500))
                if random.random() < 0.3:
                    speed = float(np.clip(speed + random.uniform(-15, 15), 0, 150))

                seq_features.append(self._encode_sensor_snapshot(lidar, fl_cam, fr_cam, back_distance, speed))

            sequences.append(np.array(seq_features, dtype=np.float32))
            final_decision = self._rule_based_from_snapshot(lidar, fl_cam, fr_cam, back_distance)
            labels.append(self._decision_to_index(final_decision))

        return sequences, labels

    def _rule_based_from_snapshot(self, lidar, fl_cam, fr_cam, back_distance):
        """Apply legacy rule logic to a single snapshot."""
        ...
        if back_distance < 50 and final_decision == "NO_Action":
            final_decision = "Full_Brake"
        elif back_distance <= 400 and final_decision == "NO_Action":
            final_decision = "Slowdown"

        return final_decision

    def _encode_sensor_snapshot(self, lidar_decision, front_left, front_right, back_distance, speed):
        lidar_index = self.DECISION_MAP.get(lidar_decision, 0)
        lidar_norm = lidar_index / (len(self.DECISION_ORDER) - 1) if len(self.DECISION_ORDER) > 1 else float(lidar_index)
        back_norm = float(np.clip(back_distance, 0.0, 500.0) / 500.0)
        speed_norm = float(np.clip(speed, 0.0, 150.0) / 150.0)
        return np.array([
            lidar_norm,
            1.0 if front_left == "Not_Free" else 0.0,
            1.0 if front_right == "Not_Free" else 0.0,
            back_norm,
            speed_norm
        ], dtype=np.float32)

    def _decision_to_index(self, decision):
        return self.DECISION_MAP.get(decision, 0)

    def _index_to_decision(self, index):
        if 0 <= index < len(self.DECISION_ORDER):
            return self.DECISION_ORDER[index]
        return "NO_Action"
    ...
    def update_decision_logic(self):
        """Update ATmega328P decision logic using temporal sequence model."""
        ...
        # Update temporal history with the latest sensor snapshot
        ...
        self.sequence_history.append(feature_vector)

        fallback_decision = self._rule_based_from_snapshot(lidar, fl_cam, fr_cam, back_dist)
        final_decision = fallback_decision
        model_used = False
        model_confidence = None

        sequence_enabled = True if self.headless_mode else self.sequence_model_enabled.get()

        if self.sequence_model and sequence_enabled and self.model_ready:
            try:
                sequence_array = np.array(self.sequence_history, dtype=np.float32)
                pred_index, confidence, _ = self.sequence_model.predict(sequence_array)
                model_confidence = confidence
                if confidence >= 0.4:
                    final_decision = self._index_to_decision(pred_index)
                    model_used = True
                else:
                    final_decision = fallback_decision
            except Exception as exc:
                self.safe_set_var(self.model_status_var, f"Model error: {str(exc)[:50]}")
                sequence_enabled = False
                final_decision = fallback_decision

        if model_confidence is not None:
            self.safe_set_var(self.model_confidence_var, f"Confidence: {model_confidence * 100:.0f}%")
            if model_used:
                self.safe_set_var(self.model_status_var, "Model: Active")
            else:
                self.safe_set_var(self.model_status_var, "Model: Low confidence - fallback")
        else:
            self.safe_set_var(self.model_confidence_var, "Confidence: N/A")
            if not sequence_enabled:
                self.safe_set_var(self.model_status_var, "Model: Disabled")
            elif not self.model_ready:
                self.safe_set_var(self.model_status_var, "Model: Initializing...")
        ...
        if self.scenario_active:
            ...
            label_for_training = ground_truth if ground_truth is not None else final_decision
            self.sequence_training_buffer.append({
                'sequence': np.array(self.sequence_history, dtype=np.float32),
                'label': self._decision_to_index(label_for_training)
            })
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (Sequence)")
