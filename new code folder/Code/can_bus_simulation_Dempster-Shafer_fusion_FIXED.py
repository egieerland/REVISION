...
class CANBusSimulator:
    def __init__(self, root, seed: int = 0, headless_mode: bool = False):
        self.root = root
        self.root.title("CAN Bus Simulation - Dempster-Shafer Fusion")
        ...
        # Start the GUI simulation immediately. In headless mode the caller
        # starts it explicitly after stress/output parameters are configured.
    ...
    def init_variables(self):
        ...
        # Controller-received sensor states. These are updated only after a CAN
        # message is successfully delivered, so packet loss and jitter can
        # influence the fusion result instead of being bypassed by direct
        # access to scenario values.
        self.rx_lidar_decision = "NO_Action"
        self.rx_front_left_camera = "Free"
        self.rx_front_right_camera = "Free"
        self.rx_back_distance = 500

        # Dempster-Shafer configuration. Equal fixed reliability is used for
        # positive hazard evidence from every sensor modality to avoid hidden
        # method-specific tuning. A clear/no-hazard reading is represented as
        # vacuous evidence because a sensor with limited coverage cannot rule
        # out hazards in another sensor's field of view.
        self.ds_frame = ("NO_Action", "Slowdown", "Partial_Brake", "Full_Brake")
        self.ds_reliability = {
            "lidar": 0.90,
            "front_left_camera": 0.90,
            "front_right_camera": 0.90,
            "back_ultrasonic": 0.90,
        }
        self.ds_total_conflict_epsilon = 1e-12
        self.last_ds_conflict = 0.0
        self.last_ds_betp = {decision: 0.25 for decision in self.ds_frame}
                    ...
                    if current_time - device['last_time'] >= effective_period:
                        ...
                        if msg:
                            if self.stress_enabled and should_drop_message(self.stress_drop_prob, self.stress_rng):
                                ...
                                continue
                            # The message reached the controller. Update the
                            # controller-side sensor state only here, after the
                            # packet-drop decision. This makes packet loss and
                            # jitter affect the actual decision input.
                            self._deliver_message_to_controller(name)
    ...
    def _reset_controller_rx_state(self):
        """Reset the controller-side view of sensor data for a new run."""
        self.rx_lidar_decision = "NO_Action"
        self.rx_front_left_camera = "Free"
        self.rx_front_right_camera = "Free"
        self.rx_back_distance = 500

    def _deliver_message_to_controller(self, device_name):
        """Update controller-side state after a CAN message is delivered.

        Sensor generator functions update the source/device state before a CAN
        frame is subjected to packet loss. Fusion must not read that source
        state directly; otherwise dropped frames would still influence the
        decision. This method is therefore called only after a frame survives
        the drop test.
        """
        if device_name == "Lidar_Controller":
            self.rx_lidar_decision = self.current_lidar_decision
        elif device_name == "Front_Left_Camera":
            self.rx_front_left_camera = self.current_front_left_camera
        elif device_name == "Front_Right_Camera":
            self.rx_front_right_camera = self.current_front_right_camera
        elif device_name == "Back_Ultrasonic":
            self.rx_back_distance = self.current_back_distance

    def _ds_discount_singleton(self, decision, alpha):
        """Create a discounted BPA for one sensor decision.

        A sensor first assigns unit mass to its local singleton decision.
        Shafer discounting then leaves alpha on that singleton and assigns the
        remaining 1-alpha to the complete frame (ignorance).
        """
        theta = frozenset(self.ds_frame)
        singleton = frozenset([decision])
        alpha = min(1.0, max(0.0, float(alpha)))
        return {singleton: alpha, theta: 1.0 - alpha}

    def _ds_sensor_masses(self, lidar, fl_cam, fr_cam, back_dist):
        """Build one BPA per delivered sensor observation.

        Positive hazard observations support the corresponding singleton
        decision. A clear observation is represented by vacuous evidence
        m(Theta)=1 because, for example, a clear front camera cannot disprove
        a rear ultrasonic hazard and vice versa.
        """
        theta = frozenset(self.ds_frame)
        vacuous = {theta: 1.0}
        masses = []

        if lidar in ("Slowdown", "Partial_Brake", "Full_Brake"):
            masses.append(self._ds_discount_singleton(
                lidar, self.ds_reliability["lidar"]
            ))
        else:
            masses.append(dict(vacuous))

        if fl_cam == "Not_Free":
            masses.append(self._ds_discount_singleton(
                "Partial_Brake", self.ds_reliability["front_left_camera"]
            ))
        else:
            masses.append(dict(vacuous))
        ...
        if fr_cam == "Not_Free":
            masses.append(self._ds_discount_singleton(
                "Partial_Brake", self.ds_reliability["front_right_camera"]
            ))
        else:
            masses.append(dict(vacuous))

        if back_dist is None or back_dist > 400:
            masses.append(dict(vacuous))
        else:
            back_decision = "Full_Brake" if back_dist < 50 else "Slowdown"
            masses.append(self._ds_discount_singleton(
                back_decision, self.ds_reliability["back_ultrasonic"]
            ))

        return masses

    def _ds_combine_two(self, m1, m2):
        """Combine two BPAs using normalized Dempster's rule."""
        conjunctive = {}
        conflict = 0.0
        ...
        for A, mass_a in m1.items():
            for B, mass_b in m2.items():
                ...
                if not intersection:
                    ...
                    conjunctive[key] = conjunctive.get(key, 0.0) + product

        denominator = 1.0 - conflict
        if denominator <= self.ds_total_conflict_epsilon:
            return None, conflict

        combined = {A: mass / denominator for A, mass in conjunctive.items()}
        # Numerical cleanup so the mass sum remains exactly interpretable.
        total = sum(combined.values())
        if total > 0:
            combined = {A: mass / total for A, mass in combined.items()}
        return combined, conflict

    def _ds_combine_all(self, masses):
        """Sequentially combine all sensor BPAs and return conflict diagnostics."""
        if not masses:
            theta = frozenset(self.ds_frame)
            return {theta: 1.0}, 0.0, False

        combined = masses[0]
        max_conflict = 0.0
        for next_mass in masses[1:]:
            combined_next, conflict = self._ds_combine_two(combined, next_mass)
            max_conflict = max(max_conflict, conflict)
            if combined_next is None:
                return combined, 1.0, True
            combined = combined_next
        return combined, max_conflict, False

    def _ds_pignistic(self, mass):
        """Convert a BPA into pignistic probabilities over singleton actions."""
        betp = {decision: 0.0 for decision in self.ds_frame}
        for subset, subset_mass in mass.items():
            if not subset:
                continue
            share = subset_mass / len(subset)
            for decision in subset:
                if decision in betp:
                    betp[decision] += share

        total = sum(betp.values())
        if total > 0:
            betp = {decision: value / total for decision, value in betp.items()}
        return betp

    def _ds_conflict_fallback(self, lidar, fl_cam, fr_cam, back_dist):
        """Conservative fallback used only for total Dempster conflict."""
        severity = {
            "NO_Action": 0,
            "Slowdown": 1,
            "Partial_Brake": 2,
        ...
        candidates = [lidar if lidar in severity else "NO_Action"]
        candidates.append("Partial_Brake" if fl_cam == "Not_Free" else "NO_Action")
        candidates.append("Partial_Brake" if fr_cam == "Not_Free" else "NO_Action")
        if back_dist is None or back_dist > 400:
            candidates.append("NO_Action")
        elif back_dist < 50:
            candidates.append("Full_Brake")
        else:
            candidates.append("Slowdown")
        return max(candidates, key=lambda d: severity[d])

    def get_scenario_ground_truth(self):
        """Return clean scenario ground truth, independent of sensing stress.

        Ground truth is derived from the original scenario event definition, not
        from noisy/dropped/delayed observations received by the fusion method.
        """
        ...
        elapsed_time = max(0.0, time.time() - self.scenario_start_time)
        current_event = None
        for event in events:
        ...
        lidar = current_event.get("lidar", "NO_Action")
        fl_cam = current_event.get("front_left", "Free")
        fr_cam = current_event.get("front_right", "Free")
        back_dist = current_event.get("back", 500)

        # Scenario truth follows the intended safety action encoded by the
        # scenario specification. It remains clean even when sensor stress is
        # enabled, preventing circular evaluation against stressed observations.
        if lidar in ("Full_Brake", "Partial_Brake", "Slowdown"):
            return lidar
    ...
    def update_decision_logic(self):
        """Fuse delivered sensor decisions using Dempster-Shafer theory."""
        # IMPORTANT: read controller-received values, not direct scenario/source
        # values. Therefore packet loss and jitter can affect the fusion input.
        lidar = self.rx_lidar_decision
        fl_cam = self.rx_front_left_camera
        fr_cam = self.rx_front_right_camera
        back_dist = self.rx_back_distance

        sensor_masses = self._ds_sensor_masses(lidar, fl_cam, fr_cam, back_dist)
        combined_mass, conflict, total_conflict = self._ds_combine_all(sensor_masses)
        self.last_ds_conflict = conflict

        if total_conflict:
            final_decision = self._ds_conflict_fallback(lidar, fl_cam, fr_cam, back_dist)
            self.last_ds_betp = {decision: 0.0 for decision in self.ds_frame}
            self.last_ds_betp[final_decision] = 1.0
        else:
            theta = frozenset(self.ds_frame)
            betp = self._ds_pignistic(combined_mass)
            self.last_ds_betp = betp

            # If every delivered sensor provides only vacuous evidence, there
            # is no positive hazard evidence and the operational decision is
            # NO_Action. Otherwise select the largest pignistic probability;
            # exact ties between hazard classes are resolved conservatively.
            if len(combined_mass) == 1 and abs(combined_mass.get(theta, 0.0) - 1.0) < 1e-12:
                ...
                final_decision = max(
                    self.ds_frame,
                    key=lambda d: (betp.get(d, 0.0), severity[d])
                )

        # Safety-critical override retained outside the evidential combination:
        # an explicit Full_Brake from LiDAR or an object closer than 50 cm at
        # the rear must not be weakened by conflicting lower-severity evidence.
        if lidar == "Full_Brake" or (back_dist is not None and back_dist < 50):
        ...
        ground_truth = self.get_scenario_ground_truth() if self.scenario_active else None
    ...
    def reset_metrics_collection(self):
        ...
        # Start each experimental run from a clean controller-side sensor state.
        self._reset_controller_rx_state()
    ...
    def collect_functional_metrics(self, current_decision, ground_truth_decision=None):
        ...
        if ground_truth_decision is None and getattr(self, 'scenario_name', 'Normal Driving') != 'Normal Driving':
            ground_truth_decision = self.get_scenario_ground_truth()
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (Dempster-Shafer Fusion)")
