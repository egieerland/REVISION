...
from xgboost_improvements import (
    enhance_features_dict,
...
class CANBusSimulator:
    ...
    def safe_set_var(self, tkinter_var, value):
        ...
        if not self.headless_mode:
            try:
                ...
                pass  # Ignore GUI update errors in edge cases
    ...
    def setup_ml_model(self):
        ...
        # Check if model already exists and load it, unless the GUI requested a refresh.
        try:
            if not self.force_retrain:
                self.model = joblib.load('can_bus_xgboost_model.pkl')
                # Check feature compatibility; retrain if mismatched
                retrain_required = False
                try:
                    booster = self.model.get_booster()
                    model_features = booster.feature_names or []
                    # Build a sample current feature frame to compare
                    sample_df = self.extract_features(
                        getattr(self, 'current_lidar_decision', 'NO_Action'),
                        getattr(self, 'current_front_left_camera', 'Free'),
                        getattr(self, 'current_front_right_camera', 'Free'),
                        getattr(self, 'current_back_distance', 250),
                        getattr(self, 'current_speed', 60.0),
                    )
                    current_features = list(sample_df.columns)
                ...
                if not retrain_required:
                    if not hasattr(self, 'smoother'):
                        self.smoother = DecisionSmoother(window=5)
                    ...
                    self.log_messages.append({
                        ...
                        'timestamp': datetime.now(),
                        'values': f'Loaded existing XGBoost model',
                        'direction': 'Event'
                    })
                    return
                else:
                    self.log_messages.append({
                        ...
                        'arbitration_id': 0x999,
                        'timestamp': datetime.now(),
                        'values': 'Model feature schema mismatch detected. Retraining model...',
                        'direction': 'Event'
                    })
            else:
                self.log_messages.append({
                    ...
                    'timestamp': datetime.now(),
                    'values': f'GUI seed applied; retraining model with seed {self.seed}',
                    'direction': 'Event'
                })
        except:
            self.log_messages.append({
                ...
                'arbitration_id': 0x999, 
                'timestamp': datetime.now(),
                'values': f'Training new XGBoost model',
                'direction': 'Event'
            })
        
        # Generate training data based on sensor scenarios
        training_data = self.generate_training_data(5000)  # Generate 5000 samples
        
        # Prepare features and target
        X = training_data.drop('decision', axis=1)
        y = training_data['decision']
        
        # Convert target to numeric
        decision_map = {"NO_Action": 0, "Slowdown": 1, "Partial_Brake": 2, "Full_Brake": 3}
        y_numeric = y.map(decision_map)
        
        # Split data
        X_train, X_test, y_train, y_test = train_test_split(
            X, y_numeric, test_size=0.2, random_state=self.seed
        )
        
        # Compute class weights emphasizing safety
        class_weights = compute_class_weights(list(y_train.values))
        sample_weight = [class_weights[int(c)] for c in list(y_train.values)]

        # Define and train the model with probabilistic output
        ...
        # Evaluate model (argmax from probabilities)
        y_pred = self.model.predict(X_test)
        accuracy = accuracy_score(y_test, y_pred)
        
        # Save model
        joblib.dump(self.model, 'can_bus_xgboost_model.pkl')
        ...
        self.log_messages.append({
            ...
            'arbitration_id': 0x999, 
            'timestamp': datetime.now(),
            'values': f'XGBoost model trained with accuracy: {accuracy:.4f}',
            'direction': 'Event'
        })
        # Initialize helpers
        self.smoother = DecisionSmoother(window=5)
    ...
    def generate_training_data(self, n_samples):
        """Generate training data based on our scenarios and sensor patterns"""
        data = []
        
        # Generate data samples based on scenario rules
        for _ in range(n_samples):
            # Generate random sensor values
            lidar_values = ["NO_Action", "Slowdown", "Partial_Brake", "Full_Brake"]
            camera_values = ["Free", "Not_Free"]
            
            lidar = random.choice(lidar_values)
            front_left_camera = random.choice(camera_values)
            front_right_camera = random.choice(camera_values)
            back_distance = random.uniform(2, 500)
            speed = random.uniform(0, 150)
            
            # Feature engineering
            is_object_front = 1 if lidar != "NO_Action" else 0
            is_object_left = 1 if front_left_camera == "Not_Free" else 0
            is_object_right = 1 if front_right_camera == "Not_Free" else 0
            back_dist_normalized = back_distance / 500  # Normalize to 0-1
            is_close_behind = 1 if back_distance < 50 else 0
            is_medium_behind = 1 if 50 <= back_distance <= 400 else 0
            speed_normalized = speed / 150  # Normalize to 0-1
            
            # Determine ground truth decision using the original rule-based logic
            decision = "NO_Action"
            ...
            # Camera inputs
            if (front_left_camera == "Not_Free" or front_right_camera == "Not_Free") and decision == "NO_Action":
                decision = "Partial_Brake"
            
            # Back ultrasonic (parking scenarios)
            if back_distance < 50 and decision == "NO_Action":
                decision = "Full_Brake"
            elif back_distance <= 400 and decision == "NO_Action":
                decision = "Slowdown"
            
            # Store features and decision
            base = {
                'lidar_no_action': 1 if lidar == "NO_Action" else 0,
                'lidar_slowdown': 1 if lidar == "Slowdown" else 0,
                'lidar_partial_brake': 1 if lidar == "Partial_Brake" else 0,
                'lidar_full_brake': 1 if lidar == "Full_Brake" else 0,
                'front_left_camera_free': 1 if front_left_camera == "Free" else 0,
                'front_right_camera_free': 1 if front_right_camera == "Free" else 0,
                'back_distance': back_dist_normalized,
                'is_object_front': is_object_front,
                'is_object_left': is_object_left,
                'is_object_right': is_object_right,
                'is_close_behind': is_close_behind,
                'is_medium_behind': is_medium_behind,
                'speed': speed_normalized,
                # synthetic persistence to teach model temporal cues
                'duration_object_front_steps': random.randint(0, 10) if is_object_front else 0,
                'duration_any_camera_blocked_steps': random.randint(0, 10) if (is_object_left or is_object_right) else 0,
            }
            enhanced = enhance_features_dict(base)
            sample = {**enhanced, 'decision': decision}
            
            data.append(sample)
        
        return pd.DataFrame(data)

    def extract_features(self, lidar, fl_cam, fr_cam, back_dist, speed):
        """Extract and engineer features for the ML model"""
        ...
        front_left_camera_free = 1 if fl_cam == "Free" else 0
        front_right_camera_free = 1 if fr_cam == "Free" else 0
        
        # Normalize numeric features
        back_dist_normalized = back_dist / 500  # Normalize to 0-1
        speed_normalized = speed / 150  # Normalize to 0-1
        
        # Feature engineering
        is_object_front = 1 if lidar != "NO_Action" else 0
        is_object_left = 1 if fl_cam == "Not_Free" else 0
        is_object_right = 1 if fr_cam == "Not_Free" else 0
        is_close_behind = 1 if back_dist < 50 else 0
        is_medium_behind = 1 if 50 <= back_dist <= 400 else 0
        
        # Create feature dictionary matching the training data
        base = {
            'lidar_no_action': lidar_no_action,
            'lidar_slowdown': lidar_slowdown,
            'lidar_partial_brake': lidar_partial_brake,
            'lidar_full_brake': lidar_full_brake,
            'front_left_camera_free': front_left_camera_free,
            'front_right_camera_free': front_right_camera_free,
            'back_distance': back_dist_normalized,
            'is_object_front': is_object_front,
            'is_object_left': is_object_left,
            'is_object_right': is_object_right,
            ...
            'is_medium_behind': is_medium_behind,
            'speed': speed_normalized,
            'duration_object_front_steps': getattr(self, 'persist_front_steps', 0),
            'duration_any_camera_blocked_steps': getattr(self, 'persist_camblk_steps', 0),
        }
        features = enhance_features_dict(base)
        
        return pd.DataFrame([features])

    def update_decision_logic(self):
        """Update ATmega328P decision logic using XGBoost model"""
        ...
        # Update persistence counters for runtime features
        self.persist_front_steps = getattr(self, 'persist_front_steps', 0)
        self.persist_camblk_steps = getattr(self, 'persist_camblk_steps', 0)
        if lidar != "NO_Action":
            self.persist_front_steps = min(10, self.persist_front_steps + 1)
        else:
            self.persist_front_steps = 0
        if (fl_cam == "Not_Free" or fr_cam == "Not_Free"):
            self.persist_camblk_steps = min(10, self.persist_camblk_steps + 1)
        else:
            self.persist_camblk_steps = 0

        # Extract features for prediction
        features_df = self.extract_features(lidar, fl_cam, fr_cam, back_dist, speed)
        
        # Align columns order to model's expected feature names if available
        try:
            booster = self.model.get_booster()
            model_features = booster.feature_names or None
            if model_features:
                # subset and order; fill missing with 0
                features_df = features_df.reindex(columns=model_features, fill_value=0)
        except Exception:
            pass
        
        # Predict using probabilities
        proba = self.model.predict_proba(features_df)[0].tolist()
        cls_idx = proba_to_decision(proba)
        decision_map = {0: "NO_Action", 1: "Slowdown", 2: "Partial_Brake", 3: "Full_Brake"}
        decision_raw = decision_map.get(cls_idx, "NO_Action")

        # Smoothing and safety guardrails
        if not hasattr(self, 'smoother'):
            self.smoother = DecisionSmoother(window=5)
        smoothed = self.smoother.step(decision_raw, proba)
        ...
        sensors_dict = {
            ...
            'fl_cam': fl_cam,
            'fr_cam': fr_cam,
            'back_dist_cm': back_dist,
        }
        final_decision = self.safety_guard.apply(sensors_dict, features_df.iloc[0].to_dict(), smoothed)
        
        # Log prediction details for analysis
        if hasattr(self, 'last_ml_prediction') and self.last_ml_prediction != final_decision:
            feature_importance = {name: value for name, value in zip(features_df.columns, self.model.feature_importances_)}
            top_features = sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)[:3]
            
            self.log_messages.append({
                'source': 'ML_MODEL', 
                'arbitration_id': 0x998, 
                'timestamp': datetime.now(),
                'values': f'Decision: {final_decision}, Top features: {top_features}',
                'direction': 'Event'
            })
        
        self.last_ml_prediction = final_decision
    ...
    def update_ui(self):
        ...
        if not self.paused:
            ...
            self.safe_set_var(self.last_lidar_decision_var, f"{self.current_lidar_decision}")
            self.safe_set_var(self.last_front_left_camera_var, f"{self.current_front_left_camera}")
            self.safe_set_var(self.last_front_right_camera_var, f"{self.current_front_right_camera}")
            self.safe_set_var(self.last_back_distance_var, f"Back:{self.current_back_distance}cm")
            
            # Update action (always show current decision)
            if self.full_brake_active:
                self.safe_set_var(self.current_action_var, "FULL_BRAKE")
            elif self.partial_brake_active:
                self.safe_set_var(self.current_action_var, "PARTIAL_BRAKE")
            elif self.slowdown_active:
                self.safe_set_var(self.current_action_var, "SLOWDOWN")
            else:
                self.safe_set_var(self.current_action_var, "NO_ACTION")
            
            # Update RPM and Speed
            self.safe_set_var(self.current_rpm_var, f"{self.current_rpm} RPM")
            self.safe_set_var(self.current_speed_var, f"{self.current_speed:.1f} km/h")
    ...
    def apply_seed_from_gui(self):
        """Apply the GUI seed value and rebuild the model."""
        try:
        ...
        try:
            self.safe_set_var(self.seed_var, str(self.seed))
        except Exception:
            pass
        ...
        self.log_messages.append({
            ...
            'arbitration_id': 0x997,
            'timestamp': datetime.now(),
            'values': f'Applied GUI seed {self.seed}',
            'direction': 'Event'
        })
    ...
    def apply_stress_from_gui(self):
        ...
        self.log_messages.append({
            'source': 'STRESS',
            'arbitration_id': 0x996,
            'timestamp': datetime.now(),
            'values': (
                f"stress_enabled={self.stress_enabled}, noise_level={self.stress_noise_level}, "
                f"drop_prob={self.stress_drop_prob}, jitter_ms={self.stress_jitter_ms}, stress_seed={self.stress_seed}"
            ),
            'direction': 'Event'
        })
    ...
    def run_robustness_stress_test(self):
        ...
        output_dir = getattr(self, 'robustness_output_dir', os.path.join(BASE_DIR, "results", "robustness_test"))
        compare_path = run_robustness_comparison(self, output_dir)
        if not getattr(self, "headless_mode", False):
            try:
                messagebox.showinfo("Robustness Test", f"Saved robustness report to:\n{compare_path}")
            except Exception:
                # In headless mode messagebox may fail; ignore
                pass
    ...
    def start_scenario(self):
        ...
        if scenario_name in self.scenarios:
            ...
            self.safe_set_var(self.scenario_progress_var, f"Running: {scenario_name}")
    ...
    def start_scenario_with_recording(self):
        ...
        if scenario_name in self.scenarios:
            ...
            self.safe_set_var(self.scenario_progress_var, f"Recording: {scenario_name}")
    ...
    def collect_system_metrics(self):
        ...
        # Memory usage (avoid psutil dependency)
        try:
            import tracemalloc
            tracemalloc.start()
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (XGBoost)")
...
if __name__ == "__main__":
    ...
    # In headless mode, run the robustness stress test automatically and exit
    if args.headless:
