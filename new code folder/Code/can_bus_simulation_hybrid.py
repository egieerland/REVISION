...
class SmHybridModel:
    """Lightweight neural network for scoring decisions."""

    def __init__(self, input_dim: int, hidden_dim: int = 16, learning_rate: float = 0.01):
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = 4  # NO_Action, Slowdown, Partial_Brake, Full_Brake
        self.learning_rate = learning_rate

        # Xavier initialization for stable training
        limit_1 = np.sqrt(6 / (self.input_dim + self.hidden_dim))
        limit_2 = np.sqrt(6 / (self.hidden_dim + self.output_dim))
        self.W1 = np.random.uniform(-limit_1, limit_1, (self.input_dim, self.hidden_dim))
        self.b1 = np.zeros(self.hidden_dim)
        self.W2 = np.random.uniform(-limit_2, limit_2, (self.hidden_dim, self.output_dim))
        self.b2 = np.zeros(self.output_dim)

    def _softmax(self, z: np.ndarray) -> np.ndarray:
        z = z - np.max(z, axis=1, keepdims=True)
        exp_z = np.exp(z)
        return exp_z / np.clip(exp_z.sum(axis=1, keepdims=True), 1e-9, None)
    ...
    def partial_fit(self, X: np.ndarray, y: np.ndarray, epochs: int = 3):
        if X.ndim == 1:
            X = X.reshape(1, -1)
        if y.ndim == 1:
            y = y.reshape(-1)
...
class CANBusSimulator:
    ...
    def init_variables(self):
        ...
        # SmHybrid status variables
        self.ml_decision_var = tk.StringVar(value="ML Decision: N/A")
        self.ml_confidence_var = tk.StringVar(value="ML Confidence: N/A")
        self.override_count_var = tk.StringVar(value="Safety Overrides: 0")
        ...
        self.metrics_collection = {
            ...
            'ml_predictions': [],  # ML predictions and confidences
            'safety_overrides': [],  # When safety rules override ML output
        }
    ...
    def setup_ml_system(self):
        """Initialize SmHybrid ML/NN system and training buffers."""
        self.decision_to_index = {
            "NO_Action": 0,
            "Slowdown": 1,
            "Partial_Brake": 2,
            "Full_Brake": 3,
        }
        self.index_to_decision = {idx: name for name, idx in self.decision_to_index.items()}

        # Features: [lidar_section_norm, fl_blocked, fr_blocked, back_norm, speed_norm, rpm_norm, scenario_flag]
        feature_count = 7
        self.hybrid_model = SmHybridModel(input_dim=feature_count, hidden_dim=24, learning_rate=0.02)
        self.ml_training_buffer = deque(maxlen=5000)
        self.override_count = 0
        self.model_update_interval = 45  # seconds
        self.last_model_update = time.time()

        # Seed the model to mirror rule-based behavior before online learning kicks in
        self.force_retrain_hybrid(include_baseline=True, show_message=False)

    def prepare_ml_features(self):
        """Prepare normalized feature vector and context for ML scoring."""
        active_section = 0
        lidar_device = self.devices.get('Lidar_Controller') if hasattr(self, 'devices') else None
        if lidar_device:
            active_section = int(lidar_device['data'].get('active_section', 0))

        lidar_section_norm = max(0.0, min(active_section, 5)) / 5.0
        front_left_blocked = 1.0 if self.current_front_left_camera == "Not_Free" else 0.0
        front_right_blocked = 1.0 if self.current_front_right_camera == "Not_Free" else 0.0
        back_distance_clamped = max(0.0, min(self.current_back_distance, 500.0))
        speed_clamped = max(0.0, min(self.current_speed, 150.0))
        rpm_clamped = max(0.0, min(self.current_rpm, 6000.0))
        scenario_flag = 1.0 if self.scenario_active else 0.0

        feature_vector = np.array([
            lidar_section_norm,
            front_left_blocked,
            front_right_blocked,
            back_distance_clamped / 500.0,
            speed_clamped / 150.0,
            rpm_clamped / 6000.0,
            scenario_flag
        ], dtype=np.float32)
        ...
        context = {
            ...
            'front_left_blocked': front_left_blocked > 0.5,
            'front_right_blocked': front_right_blocked > 0.5,
            'back_distance': back_distance_clamped,
        }

        return feature_vector, context

    def apply_safety_rules(self, context):
        """Evaluate hard safety constraints and return override decision when needed."""
        active_section = context.get('active_section', 0)
        back_distance = context.get('back_distance', 500.0)
        front_left_blocked = context.get('front_left_blocked', False)
        front_right_blocked = context.get('front_right_blocked', False)

        if active_section == 1:
            return "Full_Brake"
        if active_section in (2, 3, 4):
        ...
        if back_distance <= 400:
            return "Slowdown"
        if (front_left_blocked or front_right_blocked) and active_section == 0:
            # Cameras see obstacle while lidar is clear
            return "Partial_Brake"
        return None

    def get_ml_prediction(self, feature_vector):
        """Return ML decision, confidence, and probability distribution."""
        probabilities = self.hybrid_model.predict_proba(feature_vector)[0]
        pred_index = int(np.argmax(probabilities))
        prediction = self.index_to_decision.get(pred_index, "NO_Action")
        confidence = float(probabilities[pred_index])
        return prediction, confidence, probabilities

    def collect_training_sample(self, feature_vector, decision_name):
        """Store a sample for future training iterations."""
        label_index = self.decision_to_index.get(decision_name)
        if label_index is None:
            return
        self.ml_training_buffer.append((feature_vector.astype(np.float32), label_index))

    def maybe_train_model(self):
        """Retrain the ML model periodically using buffered samples."""
        ...
        buffer_snapshot = list(self.ml_training_buffer)
        batch_size = min(len(buffer_snapshot), 128)
        if batch_size == 0:
            return
        samples = random.sample(buffer_snapshot, batch_size)
        X = np.stack([s[0] for s in samples])
        y = np.array([s[1] for s in samples], dtype=np.int32)

        self.hybrid_model.partial_fit(X, y, epochs=5)
        self.last_model_update = time.time()

    def force_retrain_hybrid(self, include_baseline: bool = False, show_message: bool = True):
        """Retrain the hybrid model, optionally seeding it with rule-based samples."""
        if include_baseline:
            baseline_samples = [
                (np.array([0.0, 0.0, 0.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["NO_Action"]),
                (np.array([5.0 / 5.0, 0.0, 0.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["Slowdown"]),
                (np.array([2.0 / 5.0, 0.0, 0.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["Partial_Brake"]),
                (np.array([3.0 / 5.0, 1.0, 0.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["Partial_Brake"]),
                (np.array([4.0 / 5.0, 0.0, 1.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["Partial_Brake"]),
                (np.array([1.0 / 5.0, 0.0, 0.0, 1.0, 60.0 / 150.0, 3000.0 / 6000.0, 1.0], dtype=np.float32), self.decision_to_index["Full_Brake"]),
            ]
            for features, label_index in baseline_samples:
                self.ml_training_buffer.append((features, label_index))

        if not self.ml_training_buffer:
            if show_message:
                messagebox.showinfo("SmHybrid", "Not enough samples collected to retrain the model yet.")
            return

        buffer_snapshot = list(self.ml_training_buffer)
        X = np.stack([sample[0] for sample in buffer_snapshot])
        y = np.array([sample[1] for sample in buffer_snapshot], dtype=np.int32)
        epochs = 8 if include_baseline else 5
        self.hybrid_model.partial_fit(X, y, epochs=epochs)
        self.last_model_update = time.time()

        if show_message:
            messagebox.showinfo("SmHybrid", f"Model retrained with {len(y)} samples.")
    ...
    def apply_seed_from_gui(self):
        """Apply the GUI seed and rebuild the Hybrid model system."""
        try:
    ...
    # Decision Controls - removed per user request
        ...
        # SmHybrid ML status display
        hybrid_frame = ttk.LabelFrame(right_frame, text="SmHybrid Status", padding=5)
        hybrid_frame.pack(fill=tk.X, pady=5)
        ttk.Label(hybrid_frame, textvariable=self.ml_decision_var, font=('Arial', 10, 'bold')).pack(anchor=tk.W)
        ttk.Label(hybrid_frame, textvariable=self.ml_confidence_var).pack(anchor=tk.W)
        ttk.Label(hybrid_frame, textvariable=self.override_count_var).pack(anchor=tk.W)
        ttk.Button(hybrid_frame, text="Retrain Now", command=self.force_retrain_hybrid).pack(anchor=tk.W, pady=2)
    ...
    def start_simulation(self):
        ...
        def start_simulation(self):
            """Start simulation thread"""
            # Initialize the hybrid ML/NN system
            self.setup_ml_system()
    ...
    def update_decision_logic(self):
        """Update ATmega328P decision logic using ML scoring with safety constraints."""
        feature_vector, context = self.prepare_ml_features()
        safety_decision = self.apply_safety_rules(context)

        ml_prediction, confidence, probabilities = self.get_ml_prediction(feature_vector)
        self.safe_set_var(self.ml_decision_var, f"ML Decision: {ml_prediction}")
        self.safe_set_var(self.ml_confidence_var, f"ML Confidence: {confidence:.2f}")

        timestamp = time.time()
        self.metrics_collection['ml_predictions'].append({
            'timestamp': timestamp,
        ...
        if safety_decision:
            safety_idx = self.decision_to_index.get(safety_decision, 0)
            ml_idx = self.decision_to_index.get(final_decision, 0)
            if safety_idx > ml_idx:
                if safety_decision != ml_prediction:
                    override_reason = 'safety_rule'
                final_decision = safety_decision

        if confidence < 0.65:
            conservative_choice = safety_decision or "NO_Action"
            conservative_idx = self.decision_to_index.get(conservative_choice, 0)
            current_idx = self.decision_to_index.get(final_decision, 0)
            if conservative_idx < current_idx:
                final_decision = conservative_choice
                override_reason = override_reason or 'low_confidence'

        if override_reason:
            self.override_count += 1
            self.safe_set_var(self.override_count_var, f"Safety Overrides: {self.override_count}")
            self.metrics_collection['safety_overrides'].append({
                'timestamp': timestamp,
        ...
        label_for_training = ground_truth or safety_decision or final_decision
        self.collect_training_sample(feature_vector, label_for_training)
    ...
    def reset_metrics_collection(self):
        ...
        self.override_count = 0
        self.override_count_var.set("Safety Overrides: 0")
        self.ml_decision_var.set("ML Decision: N/A")
        self.ml_confidence_var.set("ML Confidence: N/A")
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (Hybrid)")
