...
class CANBusSimulator:
    ...
    def init_variables(self):
        ...
        # Reverse gear state (used to gate back ultrasonic into fusion)
        self.is_reverse_gear_active = False
    ...
    def update_decision_logic(self):
        """Bayesian fusion for sensor data (balanced priors, gated back US, temporal momentum, hysteresis)"""
        # Priors - more balanced to reduce NO_Action bias
        priors = {
            "NO_Action": 0.4,
            "Slowdown": 0.2,
            "Partial_Brake": 0.2,
        ...
        # Lidar evidence - stronger emphasis, reduce NO_Action when alert present
        if lidar == "Full_Brake":
            likelihoods["Full_Brake"] *= 0.95
            likelihoods["Partial_Brake"] *= 0.4
            likelihoods["Slowdown"] *= 0.2
            likelihoods["NO_Action"] *= 0.05
        elif lidar == "Partial_Brake":
            likelihoods["Full_Brake"] *= 0.4
            likelihoods["Partial_Brake"] *= 0.95
            likelihoods["Slowdown"] *= 0.4
            likelihoods["NO_Action"] *= 0.1
        elif lidar == "Slowdown":
            likelihoods["Full_Brake"] *= 0.1
            likelihoods["Partial_Brake"] *= 0.4
            likelihoods["Slowdown"] *= 0.95
            likelihoods["NO_Action"] *= 0.2

        # Camera evidence - if either camera blocked
        if (fl_cam == "Not_Free") or (fr_cam == "Not_Free"):
            likelihoods["Full_Brake"] *= 0.6
            likelihoods["Partial_Brake"] *= 0.8
            likelihoods["Slowdown"] *= 0.6
            likelihoods["NO_Action"] *= 0.1

        # Back ultrasonic evidence - only when reverse gear is active
        if getattr(self, 'is_reverse_gear_active', False):
            if back_dist < 50:
                likelihoods["Full_Brake"] *= 0.9
                likelihoods["Partial_Brake"] *= 0.5
                likelihoods["Slowdown"] *= 0.2
                likelihoods["NO_Action"] *= 0.05
            elif back_dist <= 400:
                likelihoods["Full_Brake"] *= 0.2
                likelihoods["Partial_Brake"] *= 0.5
                likelihoods["Slowdown"] *= 0.85
                likelihoods["NO_Action"] *= 0.2
            else:
                likelihoods["Full_Brake"] *= 0.1
                likelihoods["Partial_Brake"] *= 0.2
                likelihoods["Slowdown"] *= 0.4
                likelihoods["NO_Action"] *= 1.15

        # Temporal momentum - boost previous non-NO_Action decision slightly
        if hasattr(self, 'last_decision') and self.last_decision and self.last_decision != "NO_Action":
            if self.last_decision in likelihoods:
                likelihoods[self.last_decision] *= 1.2

        # Posterior with small-value protection
        posteriors = {}
        total = 0.0
        for d in priors:
            posteriors[d] = priors[d] * max(likelihoods[d], 1e-10)
            total += posteriors[d]
        if total > 0:
            for d in posteriors:
                posteriors[d] /= total

        # Store for inspection/visualization
        self.decision_probabilities = posteriors
        final_decision = max(posteriors, key=posteriors.get)
        self.current_confidence = posteriors.get(final_decision, 0.0)
        self.all_confidences = posteriors.copy()

        # Hysteresis - avoid flapping on near-ties
        if hasattr(self, 'last_decision') and self.last_decision and self.last_decision != final_decision:
            prev_conf = posteriors.get(self.last_decision, 0.0)
            if (self.current_confidence - prev_conf) < 0.10:
        ...
        # Ground truth derived from current sensor state
        ground_truth = None
        if self.scenario_active:
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CAN bus simulation (Bayesian Fusion)")
